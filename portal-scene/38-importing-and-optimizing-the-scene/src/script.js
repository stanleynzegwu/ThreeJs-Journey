 ...
 * Texture
 */
const portalTexture = textureLoader.load('portal-baked.jpg')
 ...
 * Model
 */
gltfLoader.load('portalm.glb',(gltf) => {
    const bakedMesh = gltf.scene.children.find(({name}) => name === 'baked')
    bakedMesh.material = bakedMaterial

    //assign material to portalLight
    const portalLightMesh = gltf.scene.children.find(({name}) => name.includes('portalLight'))
    portalLightMesh.material = portalMaterial

    //assign material to poleLight
    gltf.scene.children.filter(({name}) => name.includes('pole')).forEach((mesh) => mesh.material = poleMaterial)
